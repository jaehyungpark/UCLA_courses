...
float arrayvals[width][height][3];

/* function for multithreading */
void * multi_thread(void* num)
{
  Vec3 camera_pos;
  ...
  const double subsample_dy
	...
	: pixel_dy;
  /* for every pixel */
  for( int px = *(int*)num; px<width; px += nthreads )
  {
	...
	for( int py=0; py<height; ++py )
	{
	  ...
	  for (int j=0; j<3; j++){
	    arrayvals[px][py][j] = scaled_color[j];
	  }
	}
  }
  return NULL;
...
int main( int argc, char **argv )
{
  ...
  printf( "# Rendering scene with %d spheres and %d lights\n",
	  scene.sphere_count,
	  scene.light_count );
  int get_thread[nthreads];
  pthread_t threads[nthreads];
  for(int m = 0; m < nthreads; m++)
  {
  	get_thread[m] = m;
	int result = pthread_create(&threads[m], NULL, multi_thread,&get_thread[m]);
	if(result)
	{
  ...
  for(int j = 0; j < width; j++)
  {
	...
	{
  	  	printf("%.0f %.0f %.0f\n",
  	  		arrayvals[j][k][0], arrayvals[j][k][1], arrayvals[j][k][2]);
	}
	printf("\n");
  }
  free_scene( &scene );

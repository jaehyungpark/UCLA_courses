...
Feature* extract_feature(double* data, double* time, int* S_i, int n_S)
{
	...
	for(i = 0; i< n_S-1; i++){
		...
		integral(data, time, pos0, pos4, &features[i].integral);
...
void global_feature(double* accel_x, double* gyro_y,
	double* time, int* S_i, int n_S, char* ofile_maxmin_name, char* ofile_feature_name)
{
	...
	features_accel_x = extract_feature(accel_x, time, S_i, n_S);
	...
	fprintf(fp, "Seg0_Max,Seg0_Min,Seg1_Max,Seg1_Min,Seg2_Max,Seg2_Min,Seg3_Max,Seg3_Min,Integral,Period\n");
	for(i = 0; i < n_S-1; i++){
		fprintf(fp, "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%20.10lf\n",
			features_accel_x[i].seg0.max, features_accel_x[i].seg0.min,
			features_accel_x[i].seg1.max, features_accel_x[i].seg1.min,
			features_accel_x[i].seg2.max, features_accel_x[i].seg2.min,
			features_accel_x[i].seg3.max, features_accel_x[i].seg3.min,
			features_accel_x[i].integral, period[i]);
	}
	fclose(fp);

...
int main(int argc, char **argv) {
	...
	//Initialization
	if(argc != 7) {
		fprintf(stderr, "Usage: %s <slow run feature file> <fast run feature file> <output train file> <output scale file> <input neuron num> <output neuron num>\n",
				argv[0]);
		exit(EXIT_FAILURE);
	}
	...
	outputScaleFileName = argv[argc-3];
	outputTrainFileName = argv[argc-4];
	...
	integral_scale = max(integral, N_SAMPLES)*2;
	period_scale = max(period, N_SAMPLES)*2;

	normalize(integral, N_SAMPLES, integral_scale);
	normalize(period, N_SAMPLES, period_scale);

	fprintf(stdout, "Print integral scale %lf\n", integral_scale);
	fprintf(stdout, "Print period scale %lf\n", period_scale);
	// normalization ends
	fprintf(stdout, "Stop normalizing data...\n");

	fprintf(stdout, "%s\n...", outputScaleFileName);
	fp = fopen(outputScaleFileName, "w");
